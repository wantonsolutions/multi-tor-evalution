...
uint16_t verbose_level = 2; /**< Silent by default. */
...
init_config(void)
{
	...
	//printf("RTE_MAX_QUEUES_PER_PORT:%d", RTE_MAX_QUEUES_PER_PORT);
 ...
 */
static void
run_pkt_measure_on_lcore(struct fwd_lcore *fc, packet_fwd_t pkt_fwd)
{
	struct fwd_stream **fsm;
	streamid_t nb_fs;
	streamid_t sm_id;
	...
	do {
		for (sm_id = 0; sm_id < nb_fs; sm_id++){
			(*pkt_fwd)(fsm[sm_id]);
			printf("sm_id:%" PRIu16 ",tx_pkt:%" PRIu64 ",rx_pkt:%" PRIu64 "\n", 
				sm_id, fsm[sm_id]->tx_packets, fsm[sm_id]->rx_packets);
		}
	} while (! fc->stopped);
}
 ...
 */
static int
run_rtt_measure_on_core(void *fwd_arg)
{
	printf("run_rtt_measure_on_core\n");
	run_pkt_measure_on_lcore((struct fwd_lcore *) fwd_arg, 
					rtt_measure_fwd_engine.packet_fwd);
	return 0;
}
...
launch_info_exchange_and_forwarding(lcore_function_t *pkt_fwd_on_lcore, 
	...
	if ((interactive == 0) || (lc_id != rte_lcore_id())) {
		...
		diag = rte_eal_remote_launch(info_exchange_on_lcore,
							fwd_lcores[0], lc_id);
		if (diag != 0)
			printf("launch rtt_measure on lcore %u failed - diag=%d\n",
					lc_id, diag);
	}
...
start_packet_forwarding(int with_tx_first)
{
	...
	if(!rtt_measure_enabled)
		...
		printf("launch_info_exchange_and_forwarding\n");
		//launch_info_exchange_and_forwarding(start_pkt_forward_on_core, run_rtt_measure_on_core);
		launch_packet_forwarding(run_rtt_measure_on_core);
	}
}
